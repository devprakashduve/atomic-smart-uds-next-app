...
export default function Tooltip({ content, children, setBackground, isBackground=false,isRounded=false }: TooltipProps) {
  const tooltipClasses=classNames('p-2 text-center text-xs ',`${isBackground? 'bg-primary text-white':'border border-primary text-primary' }`, setBackground,` ${isRounded? 'rounded':''}`)
  const tipClasses=classNames('clip-bottom h-5 w-4 bg-primary',setBackground)
  return (
